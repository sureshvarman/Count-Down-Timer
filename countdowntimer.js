	 ...
	 function countDownTimer() {
			...
			if(parseInt(window['timerDays']) >  0 || parseInt(window['timerHour']) > 0 || parseInt(window['timerMinutes']) > 0 || parseInt(window['timerSeconds']) > 0) {
				if(parseInt(window['timerDays']) > 0 || parseInt(window['timerDays']) < 0) {
					$this.html(window['timerDays']+opts.Separator+window['timerHour']+opts.Separator+window['timerMinutes']+opts.Separator+window['timerSeconds']);
				}
				else {
					$this.html(window['timerHour']+opts.Separator+window['timerMinutes']+opts.Separator+window['timerSeconds']);
				}
			}
			if(parseInt(window['timerDays']) <= 0 && parseInt(window['timerHour']) <= 0 && parseInt(window['timerMinutes']) <= 0 && parseInt(window['timerSeconds']) <= 0) {
				if(!opts.incremental) {
					...
					if(days > 0){
						$this.html("+"+days+opts.Separator+hours+opts.Separator+minutes+opts.Separator+seconds);
					}
					else{
						$this.html("+"+hours+opts.Separator+minutes+opts.Separator+seconds);
					}
				}
			}
